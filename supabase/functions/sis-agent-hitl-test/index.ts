...
import {
  Agent, RunState, Usage, getGlobalTraceProvider, run, setTraceProcessors, setTracingDisabled, tool,
  type Span, type Trace, type TracingProcessor,
} from "@openai/agents";
...
function projectKey(jsonName: string, legacyName: string): string | null {
  ...
  if (value) { try { return JSON.parse(value)?.default ?? null; } catch { return null; } }
...
function clients(authorization: string) {
  ...
  return {
    user: createClient(url, publishable, { global: { headers: { Authorization: authorization } }, auth: { persistSession: false, autoRefreshToken: false } }),
...
class SisTraceProcessor implements TracingProcessor {
  ...
  async onTraceStart(trace: Trace) {
    ...
    await this.serialize(`trace:${trace.traceId}`, async () => {
      const { error } = await this.db.from("sis_agent_trace_runs").upsert({
        trace_id: trace.traceId, workflow_name: trace.name, group_id: trace.groupId,
        work_item_key: String(metadata.work_item_key ?? WORK_ITEM), observed_work_item_key: String(metadata.observed_work_item_key ?? OBSERVED_WORK_ITEM),
        environment_key: String(metadata.environment_key ?? ENVIRONMENT), status: "running", sdk_name: "@openai/agents", sdk_version: SDK_VERSION,
        source: "agents_sdk_custom_processor", started_at: new Date().toISOString(), metadata, updated_at: new Date().toISOString(),
      }, { onConflict: "trace_id" });
  ...
  async onTraceEnd(trace: Trace) {
    await this.serialize(`trace:${trace.traceId}`, async () => {
      const { error } = await this.db.from("sis_agent_trace_runs").update({ status: "completed", ended_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("trace_id", trace.traceId);
  ...
  async onSpanStart(span: Span<any>) {
    ...
    await this.serialize(`span:${span.spanId}`, async () => {
      const { error } = await this.db.from("sis_agent_trace_spans").upsert({
        span_id: span.spanId, trace_id: span.traceId, parent_span_id: span.parentId,
        span_name: String(raw?.name ?? raw?.type ?? "span").slice(0,160), span_kind: String(raw?.type ?? "unknown").slice(0,80),
        status: "running", started_at: span.startedAt ?? new Date().toISOString(), data: safeValue(raw?.data ?? {}), updated_at: new Date().toISOString(),
      }, { onConflict: "span_id" });
  ...
  async onSpanEnd(span: Span<any>) {
    ...
    await this.serialize(`span:${span.spanId}`, async () => {
      const { error } = await this.db.from("sis_agent_trace_spans").upsert({
        span_id: span.spanId, trace_id: span.traceId, parent_span_id: span.parentId,
        span_name: String(raw?.name ?? raw?.type ?? "span").slice(0,160), span_kind: String(raw?.type ?? "unknown").slice(0,80),
        status: span.error ? "failed" : "completed", started_at: span.startedAt ?? new Date().toISOString(), ended_at: span.endedAt ?? new Date().toISOString(),
        data: safeValue(raw?.data ?? {}), error_summary: span.error ? "span_failed_redacted" : null, updated_at: new Date().toISOString(),
      }, { onConflict: "span_id" });
      if (error) throw new Error(`SPAN_END_PERSIST_FAILED:${error.message}`);
    });
  }
  async forceFlush() { while (this.chains.size > 0) await Promise.allSettled([...this.chains.values()]); }
...
function serializedTraceId(state: string) { try { return String(JSON.parse(state)?.trace?.id ?? "") || null; } catch { return null; } }
...
async function authorizeCaller(req: Request, expectedWorker: string) {
  ...
  const { data: binding, error: bindingError } = await admin.from("sis_agent_runtime_bindings").select("worker_key")
    .eq("worker_key", expectedWorker).eq("environment_key", ENVIRONMENT).eq("status","active").eq("auth_subject",data.user.id).maybeSingle();
...
async function p3048Decision(db: SupabaseClient, hitlId: string) {
  const { data, error } = await db.rpc("sis_authorization_shadow_evaluate_v2", {
    p_actor: STARTER, p_environment: ENVIRONMENT, p_capability_key: CAPABILITY, p_action: ACTION_KEY, p_resource_key: RESOURCE_KEY,
    p_risk_context: { provider_write: true, approval_required: true, destructive: false, external_financial_write: false },
    p_metadata: { work_item: WORK_ITEM, phase: "phase_2_agents_sdk_hitl", hitl_id: hitlId, observed_work_item: OBSERVED_WORK_ITEM },
  });
  if (error || !data) throw new Error(`P3_048_EVALUATION_FAILED:${error?.message ?? "NO_RESULT"}`);
  if (data.decision !== "APPROVAL_REQUIRED" || data.approval_required !== true || data.shadow_only_resource !== true || data.policy_version !== "p3_048_shadow_v2") throw new Error("P3_048_APPROVAL_REQUIRED_NOT_CONFIRMED");
  return data as Record<string, any>;
}
function modelHasToolResult(input: unknown) {
  const s = JSON.stringify(input ?? []);
  return s.includes("function_call_result") || s.includes("function_call_output") || s.includes("EVIDENCE_ONLY_EXECUTED_NO_PROVIDER_WRITE") || s.includes("P3-049 TEST rejection") || s.includes("Tool execution was not approved");
}
function buildAgent(db: SupabaseClient, hitlId: string, authorizationDecision: string, resolutionClaimId: string | null = null) {
